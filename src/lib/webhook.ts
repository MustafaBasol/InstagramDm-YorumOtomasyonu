...
async function processMessagingEvent(event: MetaMessagingEvent) {
...
async function processCommentChange(
  ...
  // Skip webhook events generated by our own account to avoid loops.
...
export async function processWebhookPayload(payload: MetaWebhookPayload) {
  ...
  for (const entry of entries) {
    ...
    for (const change of entry.changes ?? []) {
      ...
      await processCommentChange(entry.id, change.value);
    }
  }
}
